<template>
  <div class="view">
    <div class="topbar">
      <div>
        <div class="topbar-title">Gastos</div>
        <div class="topbar-sub">{{ mesLabel }}</div>
      </div>
      <select v-model="mesSeleccionado" @change="cargarGastos" class="select-mes">
        <option v-for="m in meses" :key="m.id" :value="m.id">
          {{ nombreMes(m.mes) }} {{ m.anio }}
        </option>
      </select>
    </div>

    <div class="page-content">
      <div class="metrics-row">
        <div class="metric-card">
          <div class="metric-label">Total gastado</div>
          <div class="metric-val red">{{ formatCLP(totalGastos) }}</div>
        </div>
        <div class="metric-card">
          <div class="metric-label">Gastos Hormiga (Menos de $3.000)</div>
          <div class="metric-val" style="color: #F59E0B">{{ formatCLP(totalHormiga) }}</div>
        </div>
        <div class="metric-card">
          <div class="metric-label">Registros</div>
          <div class="metric-val">{{ gastos.length }}</div>
        </div>
      </div>

      <div class="two-col">
        <div class="card">
          <div class="card-title">Distribución por Categoría</div>
          <div v-if="gastos.length === 0" class="empty-state">Sin datos para graficar</div>
          <apexchart v-else type="donut" height="250" :options="donutOptions" :series="donutSeries"></apexchart>
        </div>

        <div class="card">
          <div class="card-title">Agregar gasto</div>
          <div class="form-column">
            <div class="form-group">
              <label>Nombre</label>
              <input v-model="nuevoNombre" type="text" class="input" placeholder="Ej: Supermercado Lider" />
            </div>
            <div class="form-group">
              <label>Categoría</label>
              <select v-model="nuevaCategoria" class="input">
                <option v-for="cat in categorias" :key="cat" :value="cat">{{ cat }}</option>
              </select>
            </div>
            <div class="form-row-inner">
              <div class="form-group">
                <label>Monto (CLP)</label>
              <input v-model="nuevoMonto" type="number" class="input" placeholder="Ej: 15000" min="0" @keypress="$event.key.toLowerCase() === 'e' && $event.preventDefault()" />
                <div class="aviso" v-if="mesResumen && !mesResumen.mes_cerrado && parseFloat(nuevoMonto) > ((mesResumen.saldo_disponible !== null && mesResumen.saldo_disponible !== undefined) ? mesResumen.saldo_disponible : (mesResumen.saldo_libre || 0))">⚠️ No hay saldo disponible suficiente para este gasto.</div>
              </div>
              <div class="form-group">
                <label>Fecha</label>
                <input v-model="nuevaFecha" type="date" class="input" />
              </div>
            </div>
            <button @click="agregarGasto" class="btn-primary" style="margin-top: 10px">Agregar Gasto</button>
          </div>
        </div>
      </div>

      <div class="card">
        <div class="card-title">Registro detallado del mes</div>
        <div v-if="gastos.length === 0" class="empty-state">Sin gastos registrados aún</div>
        <div v-for="g in gastosOrdenados" :key="g.id" class="gasto-row">
          <div class="gasto-dot" :style="{ background: colorCategoria(g.categoria) }"></div>
          <div class="gasto-info">
            <div class="gasto-nombre">{{ g.nombre }}</div>
            <div class="gasto-cat">{{ g.categoria }} · {{ g.fecha }}</div>
          </div>
          <span class="gasto-monto">−{{ formatCLP(g.monto) }}</span>
          <button @click="eliminarGasto(g.id)" class="btn-delete">×</button>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted, computed } from 'vue'
import { mesesService, gastosService } from '../services/api'

const meses = ref([])
const mesSeleccionado = ref(null)
const gastos = ref([])
const mesResumen = ref(null)
const nuevoNombre = ref('')
const nuevaCategoria = ref('Alimentación')
const nuevoMonto = ref('')
const nuevaFecha = ref(new Date().toISOString().split('T')[0])

const categorias = ['Alimentación', 'Transporte', 'Servicios', 'Salud', 'Educación', 'Libros', 'Entretenimiento', 'Ropa', 'Tecnología', 'Otros']
const coloresCat = {
  'Alimentación': '#185FA5', 'Transporte': '#D85A30', 'Servicios': '#854F0B',
  'Salud': '#D4537E', 'Educación': '#534AB7', 'Libros': '#1D9E75',
  'Entretenimiento': '#7F77DD', 'Ropa': '#D4537E', 'Tecnología': '#378ADD', 'Otros': '#888780'
}

const formatCLP = (n) => '$' + Math.round(n || 0).toLocaleString('es-CL')
const nombreMes = (n) => ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'][n - 1]
const colorCategoria = (cat) => coloresCat[cat] || '#888780'

// COMPUTED PARA GASTOS HORMIGA
const totalHormiga = computed(() => {
  return gastos.value.filter(g => g.monto < 3000).reduce((acc, g) => acc + g.monto, 0)
})

const totalGastos = computed(() => gastos.value.reduce((acc, g) => acc + g.monto, 0))
const gastosOrdenados = computed(() => [...gastos.value].sort((a, b) => new Date(b.fecha) - new Date(a.fecha)))

const mesLabel = computed(() => {
  const m = meses.value.find(x => x.id === mesSeleccionado.value)
  return m ? `${nombreMes(m.mes)} ${m.anio}` : ''
})

// CONFIGURACIÓN DEL GRÁFICO DONUT
const donutSeries = computed(() => {
  const map = {}
  gastos.value.forEach(g => map[g.categoria] = (map[g.categoria] || 0) + g.monto)
  return Object.values(map)
})

const donutOptions = computed(() => {
  const map = {}
  gastos.value.forEach(g => map[g.categoria] = (map[g.categoria] || 0) + g.monto)
  return {
    labels: Object.keys(map),
    colors: Object.keys(map).map(c => colorCategoria(c)),
    chart: { fontFamily: 'inherit' },
    legend: { position: 'bottom' },
    plotOptions: { pie: { donut: { size: '60%' } } },
    dataLabels: { enabled: false }
  }
})

const cargarMeses = async () => {
  const res = await mesesService.getAll()
  meses.value = res.data
  if (meses.value.length > 0) {
    mesSeleccionado.value = meses.value[meses.value.length - 1].id
    await cargarGastos()
  }
}

const cargarGastos = async () => {
  if (!mesSeleccionado.value) return
  const res = await gastosService.get(mesSeleccionado.value)
  gastos.value = res.data
  await cargarResumenMes()
}

const cargarResumenMes = async () => {
  if (!mesSeleccionado.value) return
  try {
    const res = await mesesService.getResumen(mesSeleccionado.value)
    mesResumen.value = res.data
  } catch {
    mesResumen.value = null
  }
}

const agregarGasto = async () => {
  if (!nuevoNombre.value || !nuevoMonto.value) return

  const monto = parseFloat(nuevoMonto.value)
  const disponible = mesResumen.value ? (mesResumen.value.saldo_disponible !== null && mesResumen.value.saldo_disponible !== undefined ? mesResumen.value.saldo_disponible : mesResumen.value.saldo_libre || 0) : 0

  if (mesResumen.value && !mesResumen.value.mes_cerrado && monto > disponible) {
    alert(`Saldo insuficiente. Disponible: ${formatCLP(disponible)}`)
    return
  }

  try {
    await gastosService.crear(mesSeleccionado.value, nuevoNombre.value, nuevaCategoria.value, monto, nuevaFecha.value)
    nuevoNombre.value = ''
    nuevoMonto.value = ''
    await cargarGastos()
  } catch (error) {
    const detalle = error.response?.data?.detail || 'No se pudo registrar el gasto.'
    alert(detalle)
  }
}

const eliminarGasto = async (id) => {
  await gastosService.eliminar(id)
  await cargarGastos()
}

onMounted(cargarMeses)
</script>

<style scoped>
.view { min-height: 100vh; background: #f5f5f3; }
.topbar { padding: 16px 24px; background: #fff; border-bottom: 1px solid #e5e5e3; display: flex; align-items: center; justify-content: space-between; }
.topbar-title { font-size: 16px; font-weight: 600; }
.topbar-sub { font-size: 12px; color: #888; }
.select-mes { width: 100%; padding: 12px 14px; border: 1px solid var(--border); border-radius: 14px; font-size: 14px; background: var(--bg-card); color: var(--text-primary); transition: border-color 0.2s ease, box-shadow 0.2s ease, background 0.2s ease; appearance: none; background-image: linear-gradient(45deg, transparent 50%, var(--text-secondary) 50%), linear-gradient(135deg, var(--text-secondary) 50%, transparent 50%); background-position: calc(100% - 18px) center, calc(100% - 14px) center; background-size: 8px 8px; background-repeat: no-repeat; }
.select-mes:focus { border-color: var(--accent); box-shadow: 0 0 0 5px rgba(24, 95, 165, 0.12); outline: none; }
.page-content { padding: 20px 24px; display: flex; flex-direction: column; gap: 16px; }

/* RESPONSIVE: De 4 columnas a 1 o 3 dependiendo del ancho */
.metrics-row { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
.metric-card { background: #efefed; border-radius: 10px; padding: 14px; }
.metric-label { font-size: 11px; color: #888; margin-bottom: 6px; text-transform: uppercase; }
.metric-val { font-size: 20px; font-weight: 600; }

.two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
.card { background: #fff; border: 1px solid #e5e5e3; border-radius: 12px; padding: 16px; }
.card-title { font-size: 11px; font-weight: 600; color: #888; text-transform: uppercase; margin-bottom: 14px; }

.form-column { display: flex; flex-direction: column; gap: 10px; }
.form-row-inner { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
.form-group label { font-size: 11px; color: #888; display: block; margin-bottom: 4px; }
.input { width: 100%; padding: 12px 14px; border: 1px solid var(--border); border-radius: 14px; font-size: 14px; background: var(--bg-card); color: var(--text-primary); transition: border-color 0.2s ease, box-shadow 0.2s ease, background 0.2s ease; }
.input:focus { border-color: var(--accent); box-shadow: 0 0 0 5px rgba(24, 95, 165, 0.12); outline: none; }
.btn-primary { padding: 10px; background: #185FA5; color: #fff; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; }

.gasto-row { display: flex; align-items: center; gap: 10px; padding: 10px 0; border-bottom: 1px solid #f0f0ee; }
.gasto-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.gasto-info { flex: 1; }
.gasto-nombre { font-size: 13px; font-weight: 500; }
.gasto-cat { font-size: 11px; color: #aaa; }
.gasto-monto { font-size: 14px; font-weight: 600; color: #993C1D; }
.btn-delete { background: none; border: none; color: #ccc; cursor: pointer; font-size: 18px; }

.red { color: #993C1D; }
.empty-state { text-align: center; color: #aaa; padding: 20px; font-size: 13px; }

/* ERROR RESPONSIVE TELÉFONO FIX */
@media (max-width: 768px) {
  .metrics-row { grid-template-columns: 1fr; }
  .two-col { grid-template-columns: 1fr; }
  .form-row-inner { grid-template-columns: 1fr; }
}
</style>